...
dnspod_transfer() {
    ...
    case "$__A" in
    0)
        __B="$__CMDBASE 'login_token=$username,$password&format=json' ${__URLBASE}User.Detail"
        __C=$__B
        ;;
    1)
...
describe_domain() {
    ret=0
    dnspod_transfer 0
    __TOKEN="$username,$password"
    __POST="login_token=$__TOKEN&format=json&domain=$__DOMAIN&sub_domain=$__HOST"
